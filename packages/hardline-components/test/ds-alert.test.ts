...
describe('hl-alert', () => {
  it('has role=alert on host', async () => {
    const el = await fixture<DsAlert>(html`<hl-alert tone="info">message</hl-alert>`);
    expect(el.getAttribute('role')).to.equal('alert');
  });

  it('defaults tone to info', async () => {
    const el = await fixture<DsAlert>(html`<hl-alert>message</hl-alert>`);
    expect(el.tone).to.equal('info');
  });

  it('reflects tone attribute', async () => {
    const el = await fixture<DsAlert>(html`<hl-alert tone="error">message</hl-alert>`);
    expect(el.getAttribute('tone')).to.equal('error');
  });

  it('renders header slot', async () => {
    const el = await fixture<DsAlert>(html`
      <hl-alert><span slot="header" id="hdr">Alert Title</span><p>body</p></hl-alert>
    `);
