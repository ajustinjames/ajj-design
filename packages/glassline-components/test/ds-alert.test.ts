...
describe('gl-alert', () => {
  it('has role=alert on host', async () => {
    const el = await fixture<DsAlert>(html`<gl-alert tone="info">message</gl-alert>`);
    expect(el.getAttribute('role')).to.equal('alert');
  });

  it('defaults tone to info', async () => {
    const el = await fixture<DsAlert>(html`<gl-alert>message</gl-alert>`);
    expect(el.tone).to.equal('info');
  });

  it('reflects tone attribute', async () => {
    const el = await fixture<DsAlert>(html`<gl-alert tone="error">message</gl-alert>`);
    expect(el.getAttribute('tone')).to.equal('error');
  });

  it('renders header slot', async () => {
    const el = await fixture<DsAlert>(html`
      <gl-alert><span slot="header" id="hdr">Alert Title</span><p>body</p></gl-alert>
    `);
