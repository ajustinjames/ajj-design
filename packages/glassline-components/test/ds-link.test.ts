...
describe('gl-link', () => {
  it('defaults tone to default', async () => {
    const el = await fixture<DsLink>(html`<gl-link><a href="#">link</a></gl-link>`);
    expect(el.tone).to.equal('default');
  });

  it('reflects tone attribute', async () => {
    const el = await fixture<DsLink>(html`<gl-link tone="accent"><a href="#">link</a></gl-link>`);
    expect(el.getAttribute('tone')).to.equal('accent');
  });

  it('renders slotted anchor', async () => {
    const el = await fixture<DsLink>(html`<gl-link><a id="inner" href="#">go</a></gl-link>`);
